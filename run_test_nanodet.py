...
while cap.isOpened():
    ...
    if ret:
        ...
        max_inds = np.argwhere(layer_output[0+1][:,coco_ind]>0.2)
        ...
        for ind in max_inds:
            ...
            box = np.max(np.reshape(layer_output[3+1][ind],(4,8)),axis=1)

            # Find pixel representation
            x = int((x+0.5)/cell_count*640)
            y = int((y+0.5)/cell_count*480)
            l,t,r,b = np.copy(box*[320,320,320,320]/cell_count).astype("int")  # What is the scale here?
